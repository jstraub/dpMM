...
//  double alpha = 1.0;
//  CrpMM<double> dirGMM_marg(alpha,niwMargBase,&rndGen);
//  
//  uint32_t N=20;
//  MatrixXd x(3,N);
...
//  CrpMM<double> dirGMM_samp(alpha,niwSampled,&rndGen);
//  
...
//  double alpha = 1.0;
//  CrpMM<double> dirGMM_sp(alpha,niwSp,&rndGen);
//  
//  uint32_t N=20;
//  uint32_t K=2;
//  MatrixXd x(3,N);
...
BOOST_AUTO_TEST_CASE(crpMM_vMF_test)
{
  ...
  double alpha = 1.0;
  CrpMM<double> dirvMF_sp(alpha,vMFsampled,3,&rndGen);
