...
// --------------------------------------------------------------------------
DpSubclustersSphereMM::DpSubclustersSphereMM() :
...
// --   copy constructor;
// --------------------------------------------------------------------------
DpSubclustersSphereMM::DpSubclustersSphereMM(const DpSubclustersSphereMM& that)
{
   copy(that);
}
// --------------------------------------------------------------------------
...
// --   assignment operator
// --------------------------------------------------------------------------
DpSubclustersSphereMM& DpSubclustersSphereMM::operator=(const DpSubclustersSphereMM& that)
{
   if (this != &that)
   {
      if (N>0)
...
// --------------------------------------------------------------------------
void DpSubclustersSphereMM::copy(const DpSubclustersSphereMM& that)
{
   N = that.N;
   D = that.D;
   D2 = D*D;
   ...
   for (int i=0; i<N; i++)
      if (that.params[i]!=NULL)
         params[i] = new cluster_sampledT<niwSphere_sampled>(*(that.params[i]));
...
// --------------------------------------------------------------------------
DpSubclustersSphereMM::DpSubclustersSphereMM(int _N, int _D, double* _data, double* _phi,
   double _alpha, niwSphere_sampled &_hyper, int _Nthreads, bool _useSuperclusters,
...
// --------------------------------------------------------------------------
void DpSubclustersSphereMM::initialize()
{
   ...
   for (int k=0; k<K; k++)
   {
     // indices of the instantiated clusters
      ...
      params[m] = new cluster_sampledT<niwSphere_sampled>(hyper,alpha);
...
// --------------------------------------------------------------------------
//void DpSubclustersSphereMM::write_output(mxArray* &plhs)
//{
//   linkedListNode<int>* node = alive.getFirst();
//   populate_k2z_z2k();
//
...
// --------------------------------------------------------------------------
void DpSubclustersSphereMM::populate_k2z_z2k()
{
   K = alive.getLength();
   linkedListNode<int>* node = alive.getFirst();
   for (int k=0; k<K; k++)
...
// --------------------------------------------------------------------------
void DpSubclustersSphereMM::sample_params()
{
...
// --------------------------------------------------------------------------
void DpSubclustersSphereMM::sample_superclusters()
{
   if (!useSuperclusters)
      return;
...
// --------------------------------------------------------------------------
void DpSubclustersSphereMM::sample_labels()
{
...
// --   Samples the label assignments for each data point
// --------------------------------------------------------------------------
void DpSubclustersSphereMM::propose_merges()
{
      ...
      if (merge_with[km]<0)
         for (int kn=km+1; kn<K; kn++) if (splittable[k2z[kn]])
         {
            ...
            if ((!useSuperclusters || superclusters[km]==superclusters[kn]) && merge_with[km]<0 && merge_with[kn]<0)
            {
               ...
               cluster_sampledT<niwSphere_sampled>* paramsm = params[zm];
               cluster_sampledT<niwSphere_sampled>* paramsn = params[zn];
...
// --   Samples the label assignments for each data point
// --------------------------------------------------------------------------
void DpSubclustersSphereMM::propose_splits()
{
   ...
   for (int k=0; k<K; k++)
   {
      ...
      if (always_splittable || splittable[k2z[k]])//paramsChanges[k2z[k]]/((double)params[k2z[k]]->getN())<=0.1)
      {
         int proc = omp_get_thread_num();
         int kz = k2z[k];
         cluster_sampledT<niwSphere_sampled>* paramsk = params[kz];
         int Nk = paramsk->getN();
...
//      mexPrintf("NumSplits=%d\n", num_splits);
      ...
      for (int nzh=0; nzh<N; nzh++)
      {
         if (params[nzh]==NULL)
         {
            ...
            params[nzh] = new cluster_sampledT<niwSphere_sampled>(hyper,alpha);
            temp_num_splits--;
...
void DpSubclustersSphereMM::propose_random_split_assignments()
{
   // temporary reduction array variables
   reduction_array<int> NArr(Nthreads, K*2, 0);
   reduction_array2<double> tArr(Nthreads, K*2, D, 0);
...
void DpSubclustersSphereMM::propose_random_splits()
{
...
void DpSubclustersSphereMM::propose_random_merges()
{
   vector<int> merge_with(K, -1);
   int numMerges = 0;
...
double DpSubclustersSphereMM::joint_loglikelihood()
{
   linkedListNode<int>* node = alive.getFirst();
